...
and short-answer exams, and automatically grading them. The interface uses
a custom navy-and-brass "examination hall" visual identity (see
[Visual Design](#visual-design) below) rather than default JavaFX styling.
...
The interface follows a consistent "examination hall" identity rather than
default JavaFX styling, defined as CSS custom properties (tokens) at the
top of `src/main/resources/styles.css`:

- **Palette:** deep navy (`#16233D`) for headers and primary text, warm
  brass/gold (`#B98A46`) as the single accent color (buttons, progress
  bar, focus rings, the header seal badge, the grade badge), a warm
  parchment background (`#F6F3EC`) instead of a cold gray, and muted
  crimson/forest tones for incorrect/correct states.
- **Typography:** Georgia (serif) for headers, section titles, and the
  grade badge letter; the system sans-serif stack for everything else —
  labels, buttons, table content.
- **Signature element:** a small circular monogram ("OE") appears in the
  header of every screen, built from plain shapes in `UIUtils.sealBadge()`
  rather than an external image file.
- **Alignment:** forms use `GridPane` column constraints (via
  `UIUtils.labelColumn()` / `fieldColumn()`) so every label lines up on a
  fixed-width, right-aligned column instead of drifting with label length.

All of this lives in `UIUtils` (shared header/eyebrow/divider/badge
builders) and `styles.css`, so the look can be re-themed from those two
places without touching individual screens.
